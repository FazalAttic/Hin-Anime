...
const DEFAULT_GRADIENT_TOP = "#5865F2"; // Discord blurple
const DEFAULT_GRADIENT_BOTTOM = "#23272a"; // Discord dark
...
const Profile = () => {
  ...
  const [formData, setFormData] = useState({
    username: "",
    bio: "",
    gradientColorTop: DEFAULT_GRADIENT_TOP,
    gradientColorBottom: DEFAULT_GRADIENT_BOTTOM,
  });
  const [gradientColorTop, setGradientColorTop] =
    useState(DEFAULT_GRADIENT_TOP);
  const [gradientColorBottom, setGradientColorBottom] = useState(
    DEFAULT_GRADIENT_BOTTOM
  );
  const fileInputRef = useRef(null);
  const bannerInputRef = useRef(null);
  const BACKEND_URL = "https://anime-backend-yf2m.onrender.com";
      ...
      try {
        ...
        if (userSnap.exists()) {
          ...
          setFormData({
            ...
            gradientColorTop:
              userData.profile?.gradientColorTop || DEFAULT_GRADIENT_TOP,
            gradientColorBottom:
              userData.profile?.gradientColorBottom || DEFAULT_GRADIENT_BOTTOM,
          });
          setGradientColorTop(
            userData.profile?.gradientColorTop || DEFAULT_GRADIENT_TOP
          );
          setGradientColorBottom(
            userData.profile?.gradientColorBottom || DEFAULT_GRADIENT_BOTTOM
          );
  ...
  const uploadImageToCloudinary = async (file) => {
    const sigRes = await fetch(`${BACKEND_URL}/api/signature`, {
      // Updated this line
      method: "POST",
    });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const updatedProfile = {
        ...
        photoURL,
        bannerURL,
        gradientColorTop: formData.gradientColorTop || DEFAULT_GRADIENT_TOP,
        gradientColorBottom:
          formData.gradientColorBottom || DEFAULT_GRADIENT_BOTTOM,
        updatedAt: new Date(),
  ...
  // Compute text color for profile card
  const top = profile.gradientColorTop || DEFAULT_GRADIENT_TOP;
  const bottom = profile.gradientColorBottom || DEFAULT_GRADIENT_BOTTOM;
  const profileTextColor =
    isColorLight(top) && isColorLight(bottom) ? "black" : "white";
      ...
      >
        {editMode ? (
          <motion.form
            ...
            style={{
              background: `linear-gradient(to top, ${
                formData.gradientColorTop || DEFAULT_GRADIENT_TOP
              } 0%, ${
                formData.gradientColorBottom || DEFAULT_GRADIENT_BOTTOM
              } 100%)`,
              border: `2px solid ${
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center space-x-4">
                  <div>
                    ...
                    <input
                      type="color"
                      value={formData.gradientColorTop}
                      onChange={(e) => {
                        setFormData({
                          ...formData,
                          gradientColorTop: e.target.value,
                        });
                        setGradientColorTop(e.target.value);
                      }}
                      className="w-10 h-8 ml-2 border-none bg-transparent cursor-pointer"
                      style={{ outline: "none" }}
                    />
                  </div>
                  <div>
                    ...
                    <input
                      type="color"
                      value={formData.gradientColorBottom}
                      onChange={(e) => {
                        setFormData({
                          ...formData,
                          gradientColorBottom: e.target.value,
                        });
                        setGradientColorBottom(e.target.value);
                      }}
                      className="w-10 h-8 ml-2 border-none bg-transparent cursor-pointer"
                      style={{ outline: "none" }}
                    />
                ...
                <span className="ml-2 text-xs text-gray-400">
                  Pick your card gradient colors!
                </span>
              </div>
          ...
          <motion.div
            ...
            style={{
              background: `linear-gradient(to top, ${
                profile.gradientColorTop || DEFAULT_GRADIENT_TOP
              } 0%, ${
                profile.gradientColorBottom || DEFAULT_GRADIENT_BOTTOM
              } 100%)`,
              border: `2px solid ${
                profile.gradientColorTop || DEFAULT_GRADIENT_TOP
              }80`,
